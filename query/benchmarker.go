...
func NewBenchmarkRunner() *BenchmarkRunner {
	...
	runner.statProcessor = &statProcessor{
		limit: &runner.limit,
	}
	flag.StringVar(&runner.dbName, "db-name", "benchmark", "Name of database to use for queries")
	flag.Uint64Var(&runner.statProcessor.burnIn, "burn-in", 0, "Number of queries to ignore before collecting statistics.")
	flag.Uint64Var(&runner.limit, "limit", 0, "Limit the number of queries to send, 0 = no limit")
	flag.Uint64Var(&runner.statProcessor.printInterval, "print-interval", 100, "Print timing stats to stderr after this many queries (0 to disable)")
...
func (b *BenchmarkRunner) Run(queryPool *sync.Pool, processorCreateFn ProcessorCreate) {
	...
	if b.statProcessor.burnIn > b.limit {
...
func (b *BenchmarkRunner) processorHandler(wg *sync.WaitGroup, queryPool *sync.Pool, processor Processor, workerNum int) {
	...
	for query := range b.ch {
		//processor.ProcessQuery(b.statProcessor, query)
		stats, err := processor.ProcessQuery(query, false)
		if err != nil {
			panic(err)
		}
		b.statProcessor.sendStats(stats)
		...
		if b.statProcessor.prewarmQueries {
			// Warm run
			stats, err = processor.ProcessQuery(query, true)
			if err != nil {
				panic(err)
			}
			b.statProcessor.sendStatsWarm(stats)
		}
		queryPool.Put(query)
	}
	wg.Done()
